...
namespace Sweetener.Reliability
{
    ...
    {
        private readonly InterruptableAsyncAction<T1, T2, T3, T4, T5> _action;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReliableAsyncAction{T1, T2, T3, T4, T5}"/>
        /// class that executes the given <see cref="AsyncAction{T1, T2, T3, T4, T5}"/> at most a
        ...
        /// </exception>
        public ReliableAsyncAction(AsyncAction<T1, T2, T3, T4, T5> action, int maxRetries, ExceptionPolicy exceptionPolicy, DelayPolicy delayPolicy)
            : this(action.IgnoreInterruption(), maxRetries, exceptionPolicy, delayPolicy)
        { }

        /// <summary>
        /// Initializes a new instance of the <see cref="ReliableAsyncAction{T1, T2, T3, T4, T5}"/>
        /// class that executes the given <see cref="InterruptableAsyncAction{T1, T2, T3, T4, T5}"/> at most a
        /// specific number of times based on the provided policies.
        /// </summary>
        /// <param name="action">The action to encapsulate.</param>
        /// <param name="maxRetries">The maximum number of retry attempts.</param>
        ...
        /// <paramref name="maxRetries" /> is a negative number other than <c>-1</c>, which represents an infinite number of retries.
        /// </exception>
        public ReliableAsyncAction(AsyncAction<T1, T2, T3, T4, T5> action, int maxRetries, ExceptionPolicy exceptionPolicy, ComplexDelayPolicy delayPolicy)
            : this(action.IgnoreInterruption(), maxRetries, exceptionPolicy, delayPolicy)
        { }
        ...
        /// <paramref name="maxRetries" /> is a negative number other than <c>-1</c>, which represents an infinite number of retries.
        /// </exception>
        public ReliableAsyncAction(InterruptableAsyncAction<T1, T2, T3, T4, T5> action, int maxRetries, ExceptionPolicy exceptionPolicy, DelayPolicy delayPolicy)
        ...
        /// <paramref name="maxRetries" /> is a negative number other than <c>-1</c>, which represents an infinite number of retries.
        /// </exception>
        public ReliableAsyncAction(InterruptableAsyncAction<T1, T2, T3, T4, T5> action, int maxRetries, ExceptionPolicy exceptionPolicy, ComplexDelayPolicy delayPolicy)
            : base(maxRetries, exceptionPolicy, delayPolicy)
        {
            _action = action ?? throw new ArgumentNullException(nameof(action));
        }
