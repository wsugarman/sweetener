...
namespace Sweetener.Linq
{
    static partial class Collection
    {
        ...
        public static IReadOnlyCollection<TSource> Take<TSource>(this ICollection<TSource> source, int count)
            => new TakenCollection<TSource>(source, Enumerable.Take(source, count), count);
        ...
        public static IReadOnlyCollection<TSource> Take<TSource>(this IReadOnlyCollection<TSource> source, int count)
            => new TakenReadOnlyCollection<TSource>(source, Enumerable.Take(source, count), count);

        // TODO: Implement TakeLast when moving to .NET Core

        private sealed class TakenCollection<TElement> : IReadOnlyCollection<TElement>
        {
            ...
            public TakenCollection(ICollection<TElement> source, IEnumerable<TElement> transformation, int count)
            {
                _source = source;
        ...
        private sealed class TakenReadOnlyCollection<TElement> : IReadOnlyCollection<TElement>
        {
            public int Count => Math.Min(_source.Count, _count);
            ...
            public TakenReadOnlyCollection(IReadOnlyCollection<TElement> source, IEnumerable<TElement> transformation, int count)
            {
                _source = source;
                _transformation = transformation;
                _count = count <= 0 ? 0 : count;
