...
namespace Sweetener.Reliability.Test
{
    partial class FuncExtensionsTestBase
    {
            ...
            where TDelayPolicyProxy : DelegateProxy<TDelayPolicy>
        {
            ...
            // Invoke
            using (CancellationTokenSource tokenSource = new CancellationTokenSource())
                Assert.That.ThrowsException<OutOfMemoryException>(() => invoke(reliableFunc, 42, "foo", 3.14D, 1000L, (ushort)1, (byte)255, TimeSpan.FromDays(30), 112U, Tuple.Create(true, 64UL), tokenSource.Token));
