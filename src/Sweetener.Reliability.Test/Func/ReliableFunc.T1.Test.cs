...
namespace Sweetener.Reliability.Test
{
    ...
    public sealed class ReliableFuncTest : ReliableDelegateTest<string>
    {
        ...
        {
            FuncProxy<string> func = new FuncProxy<string>(() => "Hello World");
...
#nullable enable
        ...
        private void Invoke(bool passToken)
        {
            ...
            foreach (bool addEventHandlers in new bool[] { false, true })
            {
                ...
                if (passToken)
                {
                    Invoke_Canceled      ((f, t) => f.Invoke(t), addEventHandlers);
        ...
        private void TryInvoke(bool passToken)
        {
            ...
            foreach (bool addEventHandlers in new bool[] { false, true })
            {
                ...
                if (passToken)
                {
                    Invoke_Canceled      ((f, t) => f.TryInvoke(t, out string _), addEventHandlers);
        ...
        {
            Func<ReliableFunc<string>, CancellationToken, Optional<string>> tryInvokeAsync;
            if (passToken)
        ...
        private void Invoke_Canceled(Action<ReliableFunc<string>, CancellationToken> invoke, bool addEventHandlers)
        {
            using CancellationTokenSource tokenSource = new CancellationTokenSource();
            tokenSource.Cancel();
