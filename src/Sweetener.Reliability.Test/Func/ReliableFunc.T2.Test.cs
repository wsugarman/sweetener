...
namespace Sweetener.Reliability.Test
{
    ...
    public sealed class ReliableFunc1Test : ReliableDelegateTest<string>
    {
        ...
        public void Invoke_CancellationToken()
        {
            ...
            Invoke_Canceled((reliableFunc, arg, token) => reliableFunc.Invoke(arg, token), addEventHandlers: false);
            Invoke_Canceled((reliableFunc, arg, token) => reliableFunc.Invoke(arg, token), addEventHandlers: true );
        }
        ...
        public void TryInvoke_CancellationToken()
        {
            ...
            // Ensure CancellationToken prevents additional retry
            Invoke_Canceled((reliableFunc, arg, token) => reliableFunc.TryInvoke(arg, token, out string _), addEventHandlers: false);
            Invoke_Canceled((reliableFunc, arg, token) => reliableFunc.TryInvoke(arg, token, out string _), addEventHandlers: true );
        ...
        private void Ctor_ComplexDelayPolicy(Func<Func<int, string>, int, ExceptionPolicy, ComplexDelayPolicy<string>, ReliableFunc<int, string>> factory)
        {
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, exceptionPolicy, complexDelayPolicy);
        ...
        private void Ctor_ResultPolicy_ComplexDelayPolicy(Func<Func<int, string>, int, ResultPolicy<string>, ExceptionPolicy, ComplexDelayPolicy<string>, ReliableFunc<int, string>> factory)
        {
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, resultPolicy, exceptionPolicy, complexDelayPolicy);
        ...
        private void Invoke(Func<ReliableFunc<int, string>, int, string> invoke)
        {
            ...
            {
                // Success
                Invoke_Success                ((f, arg, r) => Assert.AreEqual(r, invoke(f, arg)), addEventHandlers);
                Invoke_EventualSuccess        ((f, arg, r) => Assert.AreEqual(r, invoke(f, arg)), addEventHandlers);

                // Failure (Result)
                Invoke_Failure_Result         ((f, arg, r) => Assert.AreEqual(r, invoke(f, arg)), addEventHandlers);
                Invoke_EventualFailure_Result ((f, arg, r) => Assert.AreEqual(r, invoke(f, arg)), addEventHandlers);
                Invoke_RetriesExhausted_Result((f, arg, r) => Assert.AreEqual(r, invoke(f, arg)), addEventHandlers);

                // Failure (Exception)
                Invoke_Failure_Exception         ((f, arg, t) => Assert.That.ThrowsException(() => invoke(f, arg), t), addEventHandlers);
                Invoke_EventualFailure_Exception ((f, arg, t) => Assert.That.ThrowsException(() => invoke(f, arg), t), addEventHandlers);
                Invoke_RetriesExhausted_Exception((f, arg, t) => Assert.That.ThrowsException(() => invoke(f, arg), t), addEventHandlers);
            }
        }
        ...
        private void Invoke_Success(Action<ReliableFunc<int, string>, int, string> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_Failure_Result(Action<ReliableFunc<int, string>, int, string> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_Failure_Exception(Action<ReliableFunc<int, string>, int, Type> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_EventualSuccess(Action<ReliableFunc<int, string>, int, string> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_EventualFailure_Result(Action<ReliableFunc<int, string>, int, string> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_EventualFailure_Exception(Action<ReliableFunc<int, string>, int, Type> assertInvoke, bool addEventHandlers)
        {
            // Create a user-defined function that eventually fails after a transient result and exception
        ...
        private void Invoke_RetriesExhausted_Result(Action<ReliableFunc<int, string>, int, string> assertInvoke, bool addEventHandlers)
        {
        ...
        private void Invoke_RetriesExhausted_Exception(Action<ReliableFunc<int, string>, int, Type> assertInvoke, bool addEventHandlers)
        {
            // Create a user-defined function that eventually exhausts the maximum number of retries after transient results and exceptions
            Func<string> flakyFunc = FlakyFunc.Create<string, IOException>("Retry");
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => flakyFunc());
        ...
        private void Invoke_Canceled(Action<ReliableFunc<int, string>, int, CancellationToken> assertInvoke, bool addEventHandlers)
        {
