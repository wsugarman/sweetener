...
namespace Sweetener.Reliability.Test
{
    ...
    public sealed class ReliableFunc1Test : ReliableDelegateTest<string>
    {
        ...
        public void Ctor_DelayHandler()
            => Ctor_DelayHandler((f, m, e, d) => new ReliableFunc<int, string>(f, m, e, d));

        [TestMethod]
        public void Ctor_ComplexDelayHandler()
            => Ctor_ComplexDelayHandler((f, m, e, d) => new ReliableFunc<int, string>(f, m, e, d));

        [TestMethod]
        public void Ctor_ResultHandler_DelayHandler()
            => Ctor_ResultHandler_DelayHandler((f, m, r, e, d) => new ReliableFunc<int, string>(f, m, r, e, d));

        [TestMethod]
        public void Ctor_ResultHandler_ComplexDelayHandler()
            => Ctor_ResultHandler_ComplexDelayHandler((f, m, r, e, d) => new ReliableFunc<int, string>(f, m, r, e, d));

        [TestMethod]
        public void Ctor_Interruptable_DelayHandler()
            => Ctor_Interruptable_DelayHandler((f, m, e, d) => new ReliableFunc<int, string>(f, m, e, d));

        [TestMethod]
        public void Ctor_Interruptable_ComplexDelayHandler()
            => Ctor_Interruptable_ComplexDelayHandler((f, m, e, d) => new ReliableFunc<int, string>(f, m, e, d));

        [TestMethod]
        public void Ctor_Interruptable_ResultHandler_DelayHandler()
            => Ctor_Interruptable_ResultHandler_DelayHandler((f, m, r, e, d) => new ReliableFunc<int, string>(f, m, r, e, d));

        [TestMethod]
        public void Ctor_Interruptable_ResultHandler_ComplexDelayHandler()
            => Ctor_Interruptable_ResultHandler_ComplexDelayHandler((f, m, r, e, d) => new ReliableFunc<int, string>(f, m, r, e, d));
        ...
        private void Ctor_DelayHandler(Func<Func<int, string>, int, ExceptionHandler, DelayHandler, ReliableFunc<int, string>> factory)
        {
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => "Hello World");
...
#nullable enable
            ...
            ReliableFunc<int, string> actual = factory(func.Invoke, 37, exceptionHandler, delayHandler.Invoke);

            Ctor(actual, 37, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_ComplexDelayHandler(Func<Func<int, string>, int, ExceptionHandler, ComplexDelayHandler<string>, ReliableFunc<int, string>> factory)
        {
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => "Hello World");
            ExceptionHandler exceptionHandler = ExceptionPolicy.Fatal;
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func.Invoke, 37, exceptionHandler, delayHandler);

            Ctor(actual, 37, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_ResultHandler_DelayHandler(Func<Func<int, string>, int, ResultHandler<string>, ExceptionHandler, DelayHandler, ReliableFunc<int, string>> factory)
        {
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => "Hello World");
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func.Invoke, 37, resultHandler, exceptionHandler, delayHandler.Invoke);

            Ctor(actual, 37, resultHandler, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_ResultHandler_ComplexDelayHandler(Func<Func<int, string>, int, ResultHandler<string>, ExceptionHandler, ComplexDelayHandler<string>, ReliableFunc<int, string>> factory)
        {
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => "Hello World");
            ResultHandler<string> resultHandler = r => r == "Successful Value" ? ResultKind.Successful : ResultKind.Fatal;
            ExceptionHandler exceptionHandler = ExceptionPolicy.Fatal;
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func.Invoke, 37, resultHandler, exceptionHandler, delayHandler);

            Ctor(actual, 37, resultHandler, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_Interruptable_DelayHandler(Func<Func<int, CancellationToken, string>, int, ExceptionHandler, DelayHandler, ReliableFunc<int, string>> factory)
        {
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, exceptionHandler, delayHandler.Invoke);

            Ctor(actual, 37, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_Interruptable_ComplexDelayHandler(Func<Func<int, CancellationToken, string>, int, ExceptionHandler, ComplexDelayHandler<string>, ReliableFunc<int, string>> factory)
        {
            Func<int, CancellationToken, string> func = (arg, token) => "Hello World";
            ExceptionHandler exceptionHandler = ExceptionPolicy.Fatal;
            ComplexDelayHandler<string> delayHandler = (i, r, e) => TimeSpan.FromHours(1);
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, exceptionHandler, delayHandler);

            Ctor(actual, 37, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_Interruptable_ResultHandler_DelayHandler(Func<Func<int, CancellationToken, string>, int, ResultHandler<string>, ExceptionHandler, DelayHandler, ReliableFunc<int, string>> factory)
        {
            Func<int, CancellationToken, string> func = (arg, token) => "Hello World";
            ResultHandler<string> resultHandler = r => r == "Successful Value" ? ResultKind.Successful : ResultKind.Fatal;
            ExceptionHandler exceptionHandler = ExceptionPolicy.Fatal;
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, resultHandler, exceptionHandler, delayHandler.Invoke);

            Ctor(actual, 37, resultHandler, exceptionHandler, delayHandler);
            CtorFunc(actual, func);
        }

        private void Ctor_Interruptable_ResultHandler_ComplexDelayHandler(Func<Func<int, CancellationToken, string>, int, ResultHandler<string>, ExceptionHandler, ComplexDelayHandler<string>, ReliableFunc<int, string>> factory)
        {
            Func<int, CancellationToken, string> func = (arg, token) => "Hello World";
            ResultHandler<string> resultHandler = r => r == "Successful Value" ? ResultKind.Successful : ResultKind.Fatal;
            ExceptionHandler exceptionHandler = ExceptionPolicy.Fatal;
...
#nullable enable
            ...
            // Create a ReliableFunc and validate
            ReliableFunc<int, string> actual = factory(func, 37, resultHandler, exceptionHandler, delayHandler);
        ...
        {
            Func<ReliableFunc<int, string>, int, CancellationToken, Optional<string>> tryInvokeAsync;
            if (passToken)
        ...
        private void Invoke_Failure_Exception(Action<ReliableFunc<int, string>, int, CancellationToken, Type> assertInvoke, bool addEventHandlers)
        {
            ...
            FuncProxy<int, string> func = new FuncProxy<int, string>((arg) => throw new OutOfMemoryException());
