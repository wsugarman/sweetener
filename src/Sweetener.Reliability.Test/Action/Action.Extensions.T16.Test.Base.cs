...
namespace Sweetener.Reliability.Test
{
    partial class BaseActionExtensionsTest
    {
            ...
            where TDelayPolicyProxy : DelegateProxy<TDelayPolicy>
        {
            ...
            // Invoke
            using (CancellationTokenSource tokenSource = new CancellationTokenSource())
                Assert.That.ThrowsException<OutOfMemoryException>(() => invoke(reliableAction, 42, "foo", 3.14D, 1000L, (ushort)1, (byte)255, TimeSpan.FromDays(30), 112U, Tuple.Create(true, 64UL), new DateTime(2019, 10, 06), 321UL, (sbyte)-7, -24.68M, '!', 0.1F, Guid.Parse("53710ff0-eaa3-4fac-a068-e5be641d446b"), tokenSource.Token));
