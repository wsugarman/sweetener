...
namespace Sweetener.Reliability.Test
{
    ...
    public sealed class ReliableAction1Test : ReliableDelegateTest
    {
        ...
        {
            Action<ReliableAction<int>, int, CancellationToken> invokeAsync;
            if (passToken)
        ...
        {
            Func<ReliableAction<int>, int, CancellationToken, bool> tryInvokeAsync;
            if (passToken)
                tryInvokeAsync = (r, arg, t) => r.TryInvoke(arg, t);
            else
                tryInvokeAsync = (r, arg, t) => r.TryInvoke(arg);
        ...
        private void Invoke_Failure(Action<ReliableAction<int>, int, CancellationToken, Type> assertInvoke, bool addEventHandlers)
        {
            ...
            ActionProxy<int> action = new ActionProxy<int>((arg) => throw new OutOfMemoryException());
            ...
            // Invoke
            using (CancellationTokenSource tokenSource = new CancellationTokenSource())
                assertInvoke(reliableAction, 42, tokenSource.Token, typeof(OutOfMemoryException));
