...
namespace Sweetener.Reliability.Test
{
    ...
    public sealed class ReliableAsyncAction1Test : ReliableDelegateTest
    {
        ...
        public void Ctor_Interruptable_DelayPolicy()
            => Ctor_Interruptable_DelayPolicy((a, m, d, e) => new ReliableAsyncAction<int>(a, m, d, e));

        [TestMethod]
        public void Ctor_Interruptable_ComplexDelayPolicy()
            => Ctor_Interruptable_ComplexDelayPolicy((a, m, d, e) => new ReliableAsyncAction<int>(a, m, d, e));
        ...
        private void Ctor_DelayPolicy(Func<Func<int, Task>, int, ExceptionPolicy, DelayPolicy, ReliableAsyncAction<int>> factory)
        {
            ...
            ReliableAsyncAction<int> actual = factory(action.Invoke, 37, exceptionPolicy, delayPolicy.Invoke);

            // Validate wrapped action
            Func<int, CancellationToken, Task> actualAction = s_getAction(actual);
            action.Invoking += Expect.Arguments<int>(Arguments.Validate);

            Assert.AreEqual(0, action.Calls);
            actualAction(42, default);
            Assert.AreEqual(1, action.Calls);

            Ctor(actual, 37, exceptionPolicy, delayPolicy);
        }

        private void Ctor_ComplexDelayPolicy(Func<Func<int, Task>, int, ExceptionPolicy, ComplexDelayPolicy, ReliableAsyncAction<int>> factory)
        {
            FuncProxy<int, Task> action = new FuncProxy<int, Task>();
            ExceptionPolicy exceptionPolicy = ExceptionPolicies.Fatal;
            ...
            // Create a ReliableAsyncAction and validate
            ReliableAsyncAction<int> actual = factory(action.Invoke, 37, exceptionPolicy, delayPolicy);
        ...
        private void Ctor_Interruptable_DelayPolicy(Func<Func<int, CancellationToken, Task>, int, ExceptionPolicy, DelayPolicy, ReliableAsyncAction<int>> factory)
        {
        ...
        private void Ctor_Interruptable_ComplexDelayPolicy(Func<Func<int, CancellationToken, Task>, int, ExceptionPolicy, ComplexDelayPolicy, ReliableAsyncAction<int>> factory)
        {
            Func<int, CancellationToken, Task> action = async (arg, token) => await Operation.NullAsync().ConfigureAwait(false);
            ExceptionPolicy exceptionPolicy = ExceptionPolicies.Fatal;
            ComplexDelayPolicy delayPolicy = (i, e) => TimeSpan.FromHours(1);
            ...
            // Create a ReliableAsyncAction and validate
            ReliableAsyncAction<int> actual = factory(action, 37, exceptionPolicy, delayPolicy);
